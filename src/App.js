...
const searchTweetTerm = (searchTerm) => {
  ...
  const fullUrl = `${REACT_APP_APIURL}?q="${searchTerm}"&result_type=popular&count=${REACT_APP_MAXRESULT}`

  return fetch(fullUrl, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `bearer ${REACT_APP_APIURL}`
    }
  }).then(d => d.json())
}
...
function App() {
  ...
  const onSearchTweet = async (e) => {
    if(searchTerm.length > 0) {
      //invoke api
      //await searchTweetTerm(searchTerm)
    }
